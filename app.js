// app.js
// Vollständige Anwendung mit fixes: Header-Visibility, seasonTimeTrackingBox ID, CSP-konform (kein eval).
...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    const globalPageHeader = document.getElementById("globalPageHeader");
    ...
    function initHeader() {
      ...
      // ensure page-header (parent) is visible so child display changes apply
      if (globalPageHeader) globalPageHeader.style.display = "block";
    ...
    // FIX: ID in markup is seasonTimeTrackingBox, so query that
        ...
        .forEach((p, idx) => {
          ...
          li.innerHTML = `
            <label class="player-line" style="display:flex;align-items:center;gap:8px;width:100%;" for="${checkboxId}">
              <input id="${checkboxId}" name="${checkboxName}" type="checkbox" value="${p.name}" ${checked} style="flex:0 0 auto">
              ${numAreaHtml}
    ...
    // --- Render stats table (full) with rounded corners + totals styled like season header ---
